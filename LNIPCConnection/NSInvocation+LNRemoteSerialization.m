//
//  NSInvocation+LNRemoteSerialization.m
//  LNIPCConnection
//
...
void* _LNRemoteBlockIdentifierKey = &_LNRemoteBlockIdentifierKey;
static void* _LNCleanupIdentifierKey = &_LNCleanupIdentifierKey;
...
@interface _LNCleanUpHandler : NSObject @end
@implementation _LNCleanUpHandler
{
...
+ (instancetype)cleanUpHandlerWithBlock:(dispatch_block_t)block
{
	...
	_LNCleanUpHandler* rv = [_LNCleanUpHandler new];
...
@implementation NSInvocation (LNRemoteSerialization)

//Returns autoreleased encoded object
static id _encodeObject(id object, _LNIPCDistantObject* distantObject)
{
	if(object == nil)
	{
		return [NSNull null];
	...
	if([object isKindOfClass:NSClassFromString(@"NSBlock")])
	{
		...
		encodedObj[@"remoteIdentifier"] = [_LNIPCRemoteBlockRegistry registerRemoteBlock:object distantObject:distantObject];
	}
	else
	{
...
static void _encodeInvocation(NSInvocation* self, NSMutableDictionary* serialized, NSUInteger firstArg, _LNIPCDistantObject* distantObject)
{
...
- (NSDictionary*)_ln_serializedDictionaryForDistantObject:(_LNIPCDistantObject*)distantObject
{
	...
	{
		NSString* blockId = objc_getAssociatedObject(self, _LNRemoteBlockIdentifierKey);
...
static id _decodeObject(NSDictionary* encodedObj, LNIPCConnection* connection)
{
	id rv = nil;
	...
	if([type isEqualToString:@"block"])
	{
		...
		id localForwardingBlock = __NSMakeSpecialForwardingCaptureBlock(signature, ^(NSInvocation *inv) {
			objc_setAssociatedObject(inv, _LNRemoteBlockIdentifierKey, remoteIdentifier, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
			NSDictionary* serialized = [inv _ln_serializedDictionaryForDistantObject:nil];
		...
		objc_setAssociatedObject(localForwardingBlock, _LNCleanupIdentifierKey, [_LNCleanUpHandler cleanUpHandlerWithBlock:^{
...
static void _decodeInvocation(NSDictionary* serialized, NSInvocation* invocation, NSUInteger firstArg, LNIPCConnection* connection)
{
...
+ (instancetype)_ln_invocationWithSerializedDictionary:(NSDictionary*)serialized remoteConnection:(nonnull LNIPCConnection *)connection
{
	...
	if(blockCall)
	{
		...
		localBlock = [_LNIPCRemoteBlockRegistry remoteBlockForIdentifier:blockId distantObject:NULL];
